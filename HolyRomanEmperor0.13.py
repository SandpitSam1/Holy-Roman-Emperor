...
#TODO: Allow the player to enter "cancel" into any input to go back somewhere to prevent softlocks and annoying pushy stuff punishing mistakes
...
def confirm_orders_menu():
    ...
    if idle_armies > 0:
        ...
        print("You have some armies that have not moved yet. Do you want to have them all guard the province of " + PROVINCE_ADJ[prov_to_move_to][0] + " or do you want go back?")
        decision = int(input("Enter 1 to move them all to " + PROVINCE_ADJ[prov_to_move_to][0] + ", 2 to go back to the option select: "))
        #if they choose to move there
        if decision == 1:
            #moves the army there
            #loops over every country with an army in the province
            pl_country_armies_index_in_prov = -1
            
            for i, c in enumerate(army_locations[prov_to_move_to]):
                #if it finds the player in that list it takes a note of the index
                if c[0] == player_country_id:
                    pl_country_armies_index_in_prov = i
            
            #if it found existing player armies there it means it doesn't have to make space for the information
            if pl_country_armies_index_in_prov != -1:
                #gets the number of armies stationed in the province already and adds however many moved there
                army_locations[prov_to_move_to][pl_country_armies_index_in_prov][1] += idle_armies
            else:
                #make sure the armies are put there
                army_locations[prov_to_move_to].append([player_country_id, idle_armies])
    ...
    decision = input("Are you sure you are ready to end the turn and go to the next? (y/n): ")
    if decision == "y":
        all_options_selected = True
...
def train_armies_menu():
    ...
    while not stop_training_armies:
        ...
        print(COUNTRIES[player_country_id] + " has a forcelimit of " + str(forcelimit) + " armies before their armies become more expensive.")
        ...
        #ask how many
        armies_to_train = int(input("How many armies would you like to train? (You can enter a negative number to disband existing armies): "))
        #make sure they can pay for it
        while (armies_to_train * 10) > ducats or (armies_to_train * 1000) > manpower or armies_to_train == 0 or armies_to_train < -idle_armies:
            if (armies_to_train * 10) > ducats:
                armies_to_train = int(input("You don't have enough ducats to train that many troops, please input a valid number: "))
            elif (armies_to_train * 1000):
                armies_to_train = int(input("You don't have enough manpower to train that many troops, please input a valid number: "))
            elif armies_to_train < -idle_armies:
                armies_to_train = int(input("You are trying to disband more troops than you have idle troops availiable to disband, please input a valid number: "))
            else:
                armies_to_train = int(input("Please input a valid number: "))
        ...
        if armies_to_train < 0:
            ...
            country_data[player_country_id][1] -= 10 * armies_to_train
        ...
        stop_training_decision = int(input("Enter 1 to stop, 2 to continue: "))
        if stop_training_decision == 1:
            stop_training_armies = True
        print("")
...
def move_armies_menu():
    ...
    while not stop_moving_armies:
        #TODO: Allow the player to enter "cancel" into any input here to cancel a potential order
        ...
        if idle_armies > 0 and moved_armies > 0:
            ...
            move_or_retract = int(input("Enter 1 to move, 2 to retract: "))
            while not (move_or_retract == 1 or move_or_retract == 2):
                move_or_retract = int(input("Please enter a 1 or a 2: "))
        else:
            if idle_armies == 0:
                ...
                stop_moving_decision = input("(y/n): ").lower()
                if stop_moving_decision == "y":
        ...
        #find the availiable moves
        availiable_provinces = owned_provinces
        #add the province ids for neighbour_provinces
        for p in neighbour_provinces:
            availiable_provinces.append(p[1])
        ...
        while province_move_decision == None or (move_or_retract == 1 and not province_move_decision in availiable_provinces) or (move_or_retract == 2 and not province_move_decision in province_ids_stationed_in):
            ...
            if province_move_decision == None:
                province_move_decision = find_province_id(input("Invalid province name, please enter the name of a valid province: ").title())
            #if it's an invalid move
            elif move_or_retract == 1 and not province_move_decision in availiable_provinces:
                province_move_decision = find_province_id(input("You cannot move an army there, please enter the name of a valid province: ").title())
                ...
                province_move_decision = find_province_id(input("There are no armies you can retract from this province, please enter the name of a valid province: ").title())
            #just in case something else slips through the cracks
            else:
                province_move_decision = find_province_id(input("Invalid province, please enter the name of a valid province: ").title())
        ...
        if move_or_retract == 1:
            if idle_armies == 1:
                ...
                quantity_affected = int(input("How many of your armies would you like to move there: "))
                #makes sure there is no trickery
                while quantity_affected > idle_armies or quantity_affected < 1:
                    if quantity_affected > idle_armies:
                        quantity_affected = int(input("You do not have enough armies, please input a valid number: "))
                    else:
                        quantity_affected = int(input("Please input a valid number: "))
            ...
            if armies_stationed > 1:
                quantity_affected = int(input("How many armies would you like to retract the order to move here from: "))
                #trickery prevention
                while quantity_affected > armies_stationed or quantity_affected < 1:
                    if quantity_affected > armies_stationed:
                        quantity_affected = int(input("There are not enough armies there to retract, please input a valid number: "))
                    else:
                        quantity_affected = int(input("Please input a valid number: "))
            else:
        ...
        #actually moves armies around (finally)
        error_flag = False #lets hope we never have to use this
        #loops over every country with an army in the province
        pl_country_armies_index_in_prov = -1
        
        for i, c in enumerate(army_locations[province_affected]):
            #if it finds the player in that list it takes a note of the index
            if c[0] == player_country_id:
                pl_country_armies_index_in_prov = i
        ...
        #if it found existing player armies there it means it doesn't have to make space for the information
        if pl_country_armies_index_in_prov != -1:
            #does different things depending on whether the armies are moving or orders were retracted
            if move_or_retract == 1:
                #gets the number of armies stationed in the province already and adds however many moved there
                army_locations[province_affected][pl_country_armies_index_in_prov][1] += quantity_affected
            else:
                #removes the armies instead
                army_locations[province_affected][pl_country_armies_index_in_prov][1] -= quantity_affected
                #checks if there are now no more armies in that province and removes the section of the list if so
                if army_locations[province_affected][pl_country_armies_index_in_prov][1] < 1:
                    army_locations[province_affected].pop(pl_country_armies_index_in_prov)
            
        else:
            #if there is no space yet, make it if you're moving stuff there, and get really concerned if you're supposed to delete stuff there
            if move_or_retract == 2:
                #uh oh
                error_flag = True
            else:
                #make sure the armies are put there
                army_locations[province_affected].append([player_country_id, quantity_affected])
        
        print("")
        #reassures the player that the move was succesfull if it was, and warns the player if I'm bad at coding
        if error_flag:
            print("There was an error with the moves, this may or may not be your fault")
        else:
            message = "Your "
        ...
        stop_moving_decision = int(input("Enter 1 to stop, 2 to continue: "))
...
def display_bordering_country_info():
    ...
    for c in neighbours:
        ...
        if len(owned_provinces) == provinces:
            ...
            if len(owned_provinces) == 1:
                message += str(len(owned_provinces)) + " of which borders "
            else:
...
def display_valid_army_target_info():
    ...
    for p in availiable_provinces:
        print(PROVINCE_ADJ[p[1]][0] + ", owned by " + COUNTRIES[p[0]] + ".")
...
def display_economic_info():
    ...
    if loans < 1:
        ...
        if loans > LOAN_LIMIT - 2:
            print("You are approaching the loan limit of " + str(LOAN_LIMIT) + ", if you exceed this limit you will go bankrupt and lose.")
