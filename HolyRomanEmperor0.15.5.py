...
#manpower is gained from levies
...
def evaluate_end_of_turn():
    ...
    #deal with countries gaining income and manpower
    country_id = 0
    for c in country_data:
        c[0] += calculate_manpower_recovery(country_id)
        c[1] += calculate_balance(country_id)
        #checks if loans need to be taken
        if c[1] < 0:
            while c[1] < 0:
                c[1] += calculate_loan_size(country_id)
                c[3] += 1
                if country_id == player_country_id:
                    player_loans_taken += 1
        country_id += 1
    
    #clear army_locations
    reset_army_locations_list()
    
    #check if the player has too many loans
    if country_data[player_country_id][3] >= 5:
        end_game = True
        print("You were forced to take too many loans, so " + COUNTRIES[player_country_id] + " went bankrupt and got annexed by your more powerful and economically responsible neighbours.")
        return
    elif player_loans_taken > 0:
        print("You have taken " + str(player_loans_taken) + " loans due losing too much money.")
    
    country_id = 0
    #deal with non-player bankrupt nations
    for c in country_data:
        if c[3] >= 5 and country_id != player_country_id:
...
def evaluate_army_orders():
    ...
    for p in army_locations:
        ...
        if owner_army_index == -1:
            #...and there is just one attacker...
            if len(p) == 1:
                #...the attacker just takes all of the owner's territory there
                proince_owners[province_id][province_owner][0] = p[0][0]
            ...
            #once there is a clear winner, that winner takes all avaliable territory from the owner
            proince_owners[province_id][province_owner][0] = countries_with_largest_army[0]
            continue #move to next province after resolution
...
def evaluate_battle(province_id, country_1, country_2):
    army_1 = find_stationed_armies_count(country_1, province_id)
    army_2 = find_stationed_armies_count(country_2, province_id)
...
def confirm_orders_menu():
    ...
    if idle_armies > 0:
        ...
        decision = input("Enter 1 to move them all to " + PROVINCE_ADJ[prov_to_move_to][0] + ", 2 to go back to the option select: ")
...
def move_armies_menu():
    ...
    while not stop_moving_armies:
        ...
        #loops over every country with an army in the province
        pl_country_armies_index_in_prov = find_country_army_index(player_country_id, province_affected)
